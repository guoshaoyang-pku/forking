...
CURRENT = ("fig_v5_128x_", "fig_v5_s1_", "fig_v5_good_turing", "fig_v5_val_damage", "fig_v5_f_by_pass", "fig_v5_table_cosine", "fig_v5_dilution", "fig_v5_epoch_kernel", "fig_v5_netval", "fig_ls20ep_", "fig_sweep_v2_", "fig_ngram5_order5_", "fig_freq_", "fig_gap_vs_frequency")
...
def main():
    ...
    for p in sorted(x for x in FIG_ROOT.rglob("*") if x.is_file() and x.suffix.lower() in EXTS):
        ...
        used = refs.get(key, [])
        status = "current" if used or any(h in name for h in CURRENT) else ("outdated" if any(h in name for h in OLD) or category(p) in {"table_opt","short_epoch_b2"} else "orphan-review")
        hits = []
        for s in scripts:
    ...
    for x in figures:
        ...
        chips="".join("<span class='chip "+html.escape(v if v in ('current','outdated','orphan-review') else '')+"'>"+html.escape(v)+"</span>" for v in [x["status"]]+x["clusters"])
    ...
    stat=[(len(figures),"可视化文件"),(len(scripts),"绘图脚本"),(counts.get("current",0),"current"),(counts.get("orphan-review",0),"待复核"),(f"{sum(x['bytes'] for x in figures)/1e6:.1f} MB","图文件体积")]
