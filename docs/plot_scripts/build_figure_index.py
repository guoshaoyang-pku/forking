...
def numbered_figures():
    """Find captions and normalize them to chapter-local labels (图1-1/图A-1)."""
    out = {}
    path_re = re.compile(r"(?:figs/|docs/figs/)([^\"'<> )]+)")
    num_re = re.compile(r"图\s*([0-9]+(?:[-.]\s*[0-9]+)?|[A-H]-[0-9]+)")
    ...
    for d in list((ROOT / "docs").rglob("*.html")) + list((ROOT / "docs").rglob("*.md")):
        ...
        for m in path_re.finditer(text):
            key = m.group(1).lstrip("/")
            if key.startswith("figs/"): key = key[5:]
            hit = num_re.search(text[m.end():m.end()+500]) or num_re.search(text[max(0,m.start()-250):m.start()])
            if not hit: continue
            before = text[:m.start()]
            heads = list(heading_re.finditer(before))
            chapter = None
            if heads:
                raw = re.sub(r"<[^>]+>", "", heads[-1].group(1)).strip()
                hm = re.match(r"([0-9]+|[A-H])(?:[ .·]|$)", raw, re.I)
                if hm: chapter = hm.group(1).upper()
            raw_label = re.sub(r"\s+", "", hit.group(1)).replace(".", "-")
            if "-" in raw_label:
                label = "图" + raw_label
            elif chapter:
                chapter_counts[chapter] = chapter_counts.get(chapter, 0) + 1
                label = f"图{chapter}-{chapter_counts[chapter]}"
            else:
                label = "图" + raw_label
...
def main():
    refs, numbers, scripts = document_refs(), numbered_figures(), script_rows()
    figures = []
    for p in sorted(x for x in FIG_ROOT.rglob("*") if x.is_file() and x.suffix.lower() in EXTS):
        ...
        used = refs.get(key, [])
        labels = numbers.get(key, [])
        if not labels:
            status = "excluded-unnumbered"
        elif any(h in name for h in OLD) or category(p) in {"table_opt", "short_epoch_b2"}:
            status = "outdated"
        else:
            status = "referenced"
        hits = []
        ...
        figures.append({"path":rel(p), "file":name, "format":p.suffix[1:].lower(), "bytes":p.stat().st_size, "category":category(p), "status":status, "figure_numbers":labels, "scripts":sorted({s["path"] for s in hits}), "data_paths":sorted({x for s in hits for x in s["data_paths"]}) or ["未在脚本中声明；查看对应 run 的 summary.json / JSONL"], "clusters":sorted({x for s in hits for x in s["clusters"]}) or ["本地 docs/figs；原始集群需按 run 核对"], "used_by":used})
    ...
    cards=[]
    for x in figures:
        if x["status"] == "excluded-unnumbered":
            continue
    ...
    visible=[x for x in figures if x["status"] != "excluded-unnumbered"]
    stat=[(len(visible),"编号图"),(len(scripts),"绘图脚本"),(counts.get("referenced",0),"已编号"),(counts.get("excluded-unnumbered",0),"未编号（隐藏）"),(f"{sum(x['bytes'] for x in visible)/1e6:.1f} MB","编号图体积")]
