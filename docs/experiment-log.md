...
**⚠️ epoch 轴 >1x 点数据复用 bug（2026-09-07 用户发现，已修复）**：`--epoch_batches` 是 nested-prefix 语义（每 epoch 只读 shard 前 b 个 batch），而 1x shard 只有 337 batch——1.25x/1.5x/1.75x/2x 四点实际在**循环重读同一个 1x shard**（3 个"epoch" = 真实 3.76/4.51/5.26/6.0 个 pass），不是更长的 epoch。U 形右肢是 pass 数效应。修复：`epfix` 组（commit 9bb7c2ab）用真实多 shard 训练集（1.25x=shards 1,62；1.5x=1,61；1.75x=1,60,62；2x=1,2，batch 数取精确 floor 421/505/589/670），新建 freq_index_train1_25x/1_75x 两个索引（ngram_freq，2026-09-07），x 轴三臂 × 4 点 = 12 个新 run `s1v5_128_{y,v,input}_epf_{m}xL4_3ep`。≤1x 的 8 个点为合法 nested-prefix，保留。口径警示：≤1x 点的频率诊断用的是 1x 全集索引而训练只见前缀，gap 数字有效、freq-bin 曲线不可用于 epoch 臂；旧 input 臂 ep_1p25x–2x 四点同样带 bug，仅作历史记录。
