...
> **状态**：🟡 历史消融；两个 LR×1 补点的最终状态需以远端 summary.json 核对，本页不再把旧的“跑中”文字当作当前状态。

> **历史决策（2026-08-24）**：该专题当时采用 β₂=0.99（无动量）与表学习率 ×2（0.008）作消融基线；当前仓库 SSOT 已切换为表学习率 ×128（实际 0.0768），本页的 ×1/×2/×4 数字只作历史消融。
> 依据：β₂=0.99 · LR×2 分叉 @460 步、1000 步时已有 2.3 gap、val 健康；终点与 ×4 相同但更稳
> （无 10.8 尖峰、表 norm 1.32 vs 2.54）。LR×1 的两个补点让它慢慢跑完作对照，不再影响决策。
> 当前默认值请以 agents.md §1 为准：table_betas=(0.0,0.99)、table_lr_scale=128.0。
...
## 2b. 发现 F5 · epoch 边界「重播冲击」（2026-08-24 追查）

**现象**：β₂=0.99/0.98 + 表学习率 ×2/×4 的 1x-shard run，每个新 epoch 开头
**20-30 步后**出现 loss 上跳（峰值 +1.5~2.1），~50 步恢复；train/val 同步。
表学习率 ×1 或 β₂=0.999 时峰值 ≤0.48（×1 时 ≤0.04，不可见）。

**排除**：
- 采样错误被**物理排除**——spike 位置的输入张量与 epoch 1 同位置逐字节相同（MD5 验证）。
- 统计口径无问题——spike 延迟于边界点、双线同步、可恢复，非记账错位。

**机制（推断，三组对照支持）**：
1. 训练循环对全表执行 `exp_avg_sq.lerp_(g², 1-β₂)`，未命中行的二阶动量持续衰减。
2. 1x shard 一个 epoch ≈ 337 步：β₂=0.99 时，头部行重访时二阶动量已衰减到
   0.99³³⁷ ≈ **3%**；β₂=0.999 还剩 **71%**。
3. 头部行重访时更新量 ≈ lr/√(1-β₂)，比正常大 5-10 倍 → 头部表行被暴力改写 →
   经共享哈希行与 backbone 传播 2-3 个 batch → 边界后 20-30 步 loss 上跳。
4. 峰值逐 epoch 衰减（1.62→1.50→0.33→0.12），随记忆化加深而平息。

**含义**：
- 新标准（β₂=0.99 + ×2）= 重播冲击最猛烈的区域。「gap 最醒目」与「边界冲击最大」
  是同一个激进表写入的两面。
- 对两因素模型：ρ（记忆进度）在 epoch 内**不单调**，每次重播开头有冲击-重收敛结构。
- 待直接验证：逐步记录表更新范数 ‖ΔE‖，若机制正确应在边界后 0-20 步出现爆发峰。
...
| 历史基线 | `nglab1x_v10_input_fixed` / `nglab2x_input_v10_fv_fixed` / `nglab1x_v10_nogram_fixed` | 0.999 | 1 | 1.87/0.58/0.25 | ✅ |
...
| run | 设置 | GPU | 状态 |
|---|---|---|---|
| `nglab1x_opt_rmsprop_b2_099_lr1` | β₂=0.99 · 表 LR×1 · 1x shard · 2000 步 | 远端目录无 summary（待核） | stalled / status unknown |
| `nglab2x_opt_rmsprop_b2_099_lr1` | β₂=0.99 · 表 LR×1 · 2-epoch shard · 2000 步 | 未发现远端目录 | stalled / status unknown |

启动命令（集群临时脚本，路径不作为复现依赖）：极简 setting + `--table_lr_scale 1.0 --table_betas 0.0,0.99`，
其余与 `run_injpos.sh` 完全一致（10 步间隔、fixed val batches、freq-bin eval）。

## 4. 对主线的行动建议

1. **当前主线使用表学习率 ×128**（实际 0.0768）；本附录的 ×1/×2/×4 结果仅用于历史消融和健康性判断；
   此前基于它们的"β₂ 效应"读数按 F3 重新解释。
2. **β₂ 默认值为 0.99**；0.999 仅保留为历史对照，不改变当前标准。
