# 图 7-2：20 点 epoch 长度 scaling，epoch 2 和 epoch 3

冻结时间：2026-09-15 01:37:45 CST。20 个长度点均已完成全部 3 epochs，epoch 1/2/3 的 60 个精确边界值已全部采齐。当前主图同时展示 epoch 2（绿色圆点）和 epoch 3（橙色方点），epoch 1 为浅灰背景。队列全部标记 done，集群上本批次训练进程数为 0。

## 图片和复现

- 图片：[PNG](../../figs/main/fig_s1_epoch20_epoch2_epoch3.png) / [SVG](../../figs/main/fig_s1_epoch20_epoch2_epoch3.svg)。
- 边界数据：[CSV](../../figs/main/fig_s1_epoch20_epoch2_epoch3.csv)，60 行对应 20 个 run 的 epoch 1/2/3，全部为 observed。
...
- 采集脚本：[collect_epoch20_snapshot.py](../../plot_scripts/collect_epoch20_snapshot.py)；绘图脚本：[plot_epoch20_scaling.py](../../plot_scripts/plot_epoch20_scaling.py)，复用 [plot_epoch20_epoch2_live.py](../../plot_scripts/plot_epoch20_epoch2_live.py) 的数据审计。旧 epoch 2 单曲线仍可由后者复现。
...
.venv/bin/python docs/plot_scripts/plot_epoch20_scaling.py
```
...
每个点使用 seed 42、trigram-only、input 注入、clean R=2^20、table RMSProp(0,0.99)/LR=128×、backbone LR=0.0006、warmup_constant(100)、vanilla 8L/6H/768D、bf16/no compile。fixed val 为 4 batches，日志每 10 步及精确 epoch 边界采样。epoch 2/3 的取值 step 分别严格等于 `2 × epoch_batches` 和 `3 × epoch_batches`。
...
epoch 3 已补入主图：1× 在 step 1011 为 2.504998，4× 在 step 4044 为 1.232307，10× 在 step 10110 为 0.352578，15× 在 step 15165 为 0.306477，20× 在 step 20220 为 0.220282。上述 run_id 分别为 `s1v5_128_ep20_tri_{1p0,4p0,10p0,15p0,20p0}xL4_3ep_v2`，seed 均为 42。20 个长度上的 epoch 3 raw gap 均高于同长度的 epoch 2；两条曲线均为各自边界的原始 gap，未相减。
