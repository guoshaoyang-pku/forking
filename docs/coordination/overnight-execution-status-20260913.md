...
- **Repeatable coordination checks**: `overnight-dispatch-manifest.json` now records all D1–D27 owners, dependencies, parallelism, acceptance criteria, and approval gates. `feishu-marker-audit-20260913.json` snapshots TODO rev 2335 (27 markers) and paper rev 1082 (17 markers) without cloud writes. `check_navigation_links.py` checks 31 local navigation links with `missing=0`; earlier lower counts are superseded.
